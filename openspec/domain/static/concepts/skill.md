...
Every SKILL.md begins with a YAML frontmatter block declaring `name`, `description`, `agent`, `triggers`, and `agents` (all required), plus optional `args` and `version` fields. The `description` and `agent` fields are compatible with the GitHub Copilot prompt file schema (`agent` values: `ask` | `agent` | `plan`), making SKILL.md a superset of that format. The prose body contains plain markdown instructions that any instruction-following AI can act on; Claude-specific tool hints may appear as HTML comments (`<!-- Claude: use the Read tool -->`).
